...
type ReceiptTransaction = { readonly discordInteractionReceipt: {
  readonly createMany: (input: Prisma.DiscordInteractionReceiptCreateManyArgs) => Promise<Prisma.BatchPayload>;
  readonly findUnique: (input: Prisma.DiscordInteractionReceiptFindUniqueArgs) => Promise<{ readonly terminalResult: Prisma.JsonValue } | null>;
} };
...
export async function recordDiscordInteractionReceipt(transaction: ReceiptTransaction, input: ReceiptWrite): Promise<{ readonly kind: "inserted" | "replayed"; readonly terminalResult: Prisma.JsonValue }> {
  ...
  return { kind: insertion.count === 1 ? "inserted" : "replayed", terminalResult: receipt.terminalResult };
}

export async function recordDiscordReservationDecision(transaction: Prisma.TransactionClient, input: Readonly<{ decision: string; discordActorId: string; localActorId: string; now: Date; reservationId: string }>): Promise<boolean> {
  const result = await transaction.discordReservationMessage.updateMany({
    data: {
      decidedAt: input.now,
