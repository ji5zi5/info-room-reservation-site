...
describe("maintenance cleanup service", () => {
  it.each(categories)("drains 250 expired %s in three bounded batches", async (_name, method, resultField) => {
    ...
    const result = await runMaintenanceCleanup({ now, store });
  ...
  it.each(categories)("fails fast when %s expiry throws", async (_name, method) => {
    ...
    await expect(runMaintenanceCleanup({ now, store })).rejects.toThrow(`failed:${method}`);
  ...
  it("lets a retention exception win after backlog is detected", async () => {
    ...
    await expect(runMaintenanceCleanup({ now, store })).rejects.toThrow("retention failed");
...
type StoreOverrides = Partial<Record<ExpiryMethod, MaintenanceCleanupStore[ExpiryMethod]>>;
...
function storeWithOverrides(calls: string[], overrides: StoreOverrides, failRetention = false): MaintenanceCleanupStore {
...
function callBatch(
  name: ExpiryMethod,
  calls: string[],
  operation: MaintenanceCleanupStore[ExpiryMethod]
): MaintenanceCleanupStore[ExpiryMethod] {
  return async (value) => {
    calls.push(name);
