...
describe("operational FOMO harness contract", () => {
  it("pins the development-only embedded PostgreSQL package and QA commands", () => {
    ...
    const harness = readFileSync(join(process.cwd(), "scripts", "operational-fomo-harness.mjs"), "utf8");
    ...
    expect(harness).toMatch(/terminateOwnedProcessTree[\s\S]*waitForProcessGroupExit[\s\S]*SIGKILL/u);
  });
